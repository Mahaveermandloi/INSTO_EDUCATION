...
const Banner = () => {
      ...
      try {
        ...
        if (accessToken) {
          const response = await axios.get(
            "http://localhost:8000/show-banner",
            {
              headers: {
    ...
    if (selectedFile) {
      ...
      try {
        ...
        const response = await axios.post(
          "http://localhost:8000/upload-banner-image",
          formData,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
    ...
    if (window.confirm("Are you sure you want to delete this image?")) {
      try {
        ...
        const response = await axios.delete(
          `http://localhost:8000/delete-banner-image/${id}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
  ...
  return (
    <div className="lg:w-10/12 lg:ml-auto">
      <div>
        <h1 className="text-2xl lg:text-4xl my-5 font-bold">Banner</h1>
      </div>
